"""JSON-file run store with secret redaction and per-run archives.

Runs are stored in ``artifacts/runs.json`` by default, keyed by ``run_id``.
For auditability, a redacted per-run archive is also persisted to
``artifacts/run_logs/run_<run_id>.json``.
"""
...
RUNS_DIR = os.getenv("AGENTFORCE_RUNS_DIR", "artifacts/run_logs")
REDACTED = "[REDACTED]"
...
class RunStore:
    """Persists evaluation runs to a JSON file; events are in-memory + archived."""
    ...
    async def init(self) -> None:
        """Create backing files/directories and sanitize historical payloads."""
    ...
    async def finish_run(self, run_id: str, results: List[dict], metadata: Optional[Dict] = None) -> None:
        ...
        if run_id in runs:
            ...
            if metadata:
                runs[run_id].update(redact_secrets(metadata))
            self._write(runs)
    ...
    def _write(self, data: dict) -> None:
        ...
        with open(self.runs_file, "w", encoding="utf-8") as handle:
            json.dump(redact_secrets(data), handle, indent=2)
    ...
    def _write_run_archive(self, run_id: str, run_data: dict, *, include_events: bool) -> None:
        ...
        with self._run_archive_path(run_id).open("w", encoding="utf-8") as handle:
    ...
    def _sanitize_existing_archives(self) -> None:
        for archive_path in self.runs_dir.glob("run_*.json"):
            ...
            with archive_path.open("w", encoding="utf-8") as handle:
                json.dump(redact_secrets(payload), handle, indent=2)
